...
// gowrapper: https://github.com/Ythosa/gowrapper
...
var overridedInterfaceNames = map[string]string{
	"UseCase": "usecase",
}

func (builder *OptionsBuilder) buildFileName(interfaceName string) string {
	if fn, ok := overridedInterfaceNames[interfaceName]; ok {
		return fn
	}
	...
	for i, r := range interfaceName {
		if unicode.IsUpper(r) && i >= 1 && !unicode.IsUpper(rune(interfaceName[i-1])) {
			result.WriteRune('_')
		}
